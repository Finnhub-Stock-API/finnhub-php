...
[**bankBranch()**](DefaultApi.md#bankBranch) | **GET** /bank/branch | Bank Branch
...
```php
bankBranch($symbol): \Finnhub\Model\BankBranch
```

Bank Branch

Get a list of bank branches for a given bank symbol.
...
$symbol = 'JPM'; // string | Symbol of the bank, e.g. JPM

try {
    $result = $apiInstance->bankBranch($symbol);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling DefaultApi->bankBranch: ', $e->getMessage(), PHP_EOL;
}
```
...
Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **symbol** | **string**| Symbol of the bank, e.g. JPM. |

### Return type

[**\Finnhub\Model\BankBranch**](../Model/BankBranch.md)
...
```php
newsSentiment($symbol, $from, $to): \Finnhub\Model\SocialSentiment
```
...
try {
    $result = $apiInstance->newsSentiment($symbol, $from, $to);
    print_r($result);
} catch (Exception $e) {
